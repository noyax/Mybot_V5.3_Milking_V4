			...
			If $body <> "" Or $iden <> "" Then
				...
				Switch $body[$x]
					Case "BOT HELP"
						...
						$txtHelp &= '\n' & "BOT <Village Name> SWITCH <New Village Name> - switches Village name"
						$txtHelp &= '\n'
						$txtHelp &= '\n' & "Examples:"
						$txtHelp &= '\n' & "Bot MyVillage Pause"
						$txtHelp &= '\n' & "Bot Delete "
					...
					Case Else ;chat bot
						...
						Else
							...
							$startCmd2 = StringLeft($body[$x], StringLen("BOT " & StringUpper($iOrigPushB) & " SWITCH "))
							If $startCmd = "BOT " & StringUpper($iOrigPushB) & " GETCHATS " Then
								...
								EndIf
							ElseIf $startCmd2 = "BOT " & StringUpper($iOrigPushB) & " SWITCH " Then
								SetLog("Pushbullet: Received request to change profile", $COLOR_GREEN)
								$switchToProfile = StringRight($body[$x], StringLen($body[$x]) - StringLen("BOT " & StringUpper($iOrigPushB) & " SWITCH "))
								$switchToProfile = StringUpper($switchToProfile)
								If $switchToProfile = $iOrigPushB Then
								_Push($iOrigPushB & " | Profile already applied, no action was taken.")
								Else
									...
									Local $originalPrile = $iOrigPushB;
									...
									If $checkProfileExist = True Then
										...
										cmbProfile()
										_Push($iOrigPushB & " | Bot profile changed from #" & $originalPrile)
									Else
										_Push($iOrigPushB & " | Profile doesn't exit, no action was taken.")
